---
title: "TEST! Are Ontologies Trees or Lattices?"
id: 000800
...
language: en
word_count: 9571
source: raw/articles/000800.xml
---
...
The context in which we are aiming to give an answer to our question is that of ontologies applied to annotations and annotation schemes. Our examples are taken from use of the textual annotation tool CATMA #gius_etal2022[^: This paper began as a contribution to the 3d forTEXT workshop Non-hierarchical c]
...
Ontology**New international dictionary of the English language**The science of being or reality; the branch of knowledge that investigates the nature, essential properties, and relations of being, as such#webster1923


For purposes of the present discussion, we will apply the name ontology


One useful tool for deciding what exists is the so-called criterion of ontological commitment. According to Quine, a theory is committed to those and only those entities to which the bound variables of the theory must be capable of referring in order that the affirmations made in the theory be true#quine1948[^: Some philosophers will object that ontology as pursued by Quine is not at all th][^: The philosopher Barry Smith suggests that [m]ost prominent information-systems o]
...
CATMAComputer Assisted Text Markup and Annotation[^: As of December 2019, the number of estimated active users was more than 8,000 .]#gius_etal2022
...
As noted [above](#definitions)[^: A brief digression may be necessary here. In the usual case, the properties used]
...
Often, the properties we wish to discuss hold not of a single individual but of two or more individuals as a group. In symbolic logic, such properties are conventionally represented using predicates which take more than one argument. A sentence might take the form `P(`*x*`,`*y*`)``P(`*x*`,`*y*`,`*z*`)``P(`*x*`)`[above](#definitions)
...
In its current version, CATMA requires that tag sets be organized into sets of hierarchically ordered classes. But nothing in the definitions of ontology and classification schemes offered [above](#definitions)


There is however a fairly widespread idea that a properly specified ontology *ought*All ontologies include one or more central backbone hierarchies in which the relation of subtype (aka is_a) connect the nodes. Each backbone hierarchy has a single root node#rudnicki_etal2016[^: The paper, which recommends that ontologies should be organized as networks cons]
