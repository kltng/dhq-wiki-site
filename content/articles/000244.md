...
University’s **Mapping the Republic of Letters****Cultures of Knowledge**
...
institutional structures*transference*from bibliographical to digital
...
pre-processing and statistical a]
...
August 1637[/DATE]**Lycidas**Christ’s College
...
statistical model – in this case, our document-count matrix *p*×*p* correlation matrix *θ̂*, where two
...
words – excluding named entities – as their bag of words[^: Our R code for the topic modeling is at https://github.com/sdfb/sdfb_network/tre]historical significance
...
Our statistical approach follows the model of G.I. Allen and Z. Liu *p* nodes and E is the set of pairwise edges. We
...
conditional independence holds if and only if ∑jk-1=0 -1.
...
penalized likelihood can be maximized efficiently for large *p* using a graphical lasso  on the remaining variables Y∖j=Y∖{j}. We estimate σjk-1 = 0 if the estimated regression coefficients
...
conditional Poisson distribution $$p\left( Y_{j} \middle| Y_{\smallsetminus j} = \mathcal{Y}_{\smallsetminus j} \right)\sim\text{Poisson}\left( \exp\left( \theta_{j} + \sum_{k \neq j}^{}{\theta_{\text{jk}}\mathcal{Y}_{k}} \right) \right)$$

The Poisson Markov random field implied by this relationship is not amenable
to inferring network structures, as it requires θjk ≤ 0 for all pairs {j,k} \(\log{\left( \mathbb{E}\left\lbrack Y_{j} \right.\  \middle| \left. \ Y_{\smallsetminus j} = \mathcal{Y}_{\smallsetminus j} \right\rbrack \right) = \theta_{j} + \sum_{k \neq j}^{}{\theta_{\text{jk}}\mathcal{Y}_{k}}}\)
...
penalized regression method similar to the graphical lasso *j*’s counts
...
their goal is to determine which genes are connected*p* names in *n*
...
specifically, we want to know whether each edge is inout

Our confidence estimates for a specific value of ϱ correspond to a single
point on the stability paths mentioned in Meinshausen and Bühlmann  variables
