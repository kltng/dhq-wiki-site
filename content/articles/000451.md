---
title: "Calamari − A High-Performance Tensorflow-based
...
date: 2020-06-19
type: article
keywords: ["digitization", "annotation", "tools", "transcription", "graphic design", "archives"]
author_keywords: []
language: en
word_count: 4994
source: raw/articles/000451.xml
---

# Calamari − A High-Performance Tensorflow-based
                    Deep Learning Package for Optical Character Recognition

**Authors:** Christoph Wick, Christian Reul, Frank Puppe

**Volume 014, Issue 2** (2020-06-19) | article | 4,994 words

**Keywords:** [[archives-access]], annotation, [[tools-infrastructure-dhq]], transcription, [[media-studies-dhq]], [[archives-access]]
...
Optical Character Recognition (OCR) of contemporary printed fonts is widely
                    ...
                    books, a book or font specific model must be trained to achieve CERs below 2%
                        #reul2017a#springman2017


Calamari[^: https://github.com/Calamari-OCR][^: Throughout this
                        paper, the term “predict” is used whenev]#breuel2017#wick2018[[Reul et al. 2018a|Reul et al., 2018a]][[Wick et al. 2018|Wick et al., 2018]][^: https://pypi.org/project/calamari_ocr/]
...
Currently, there exist several versions of OCRopy which was originally published
                    by Breuel (2008) (see also [[Breuel et al. 2013|Breuel et al., 2013]][^: https://github.com/tmbdev/ocropy]#graves2006


Kraken[^: http://kraken.re/][^: https://pytorch.org/]


While OCRopy is still maintained OCRopy 2[^: https://github.com/tmbdev/ocropy2][^: https://github.com/NVlabs/ocropus3]
...
A general approach to improve the accuracy of a model on a specific dataset
                        ...
                        model and to finetune for the specific task (see e.g. [[Reul et al. 2017b|Reul et al., 2017b]]@
...
Another technique to obtain more robust results is to vote the outcomes of
                        different models (see e.g. [[Reul et al. 2018b|Reul et al.,
                        2018b]]confidence
                            voting
...
In case of OCR, confidence voting showed the best results so far. This voting
                        ...
                        alternatives with its probabilities into account. [Figure 1](#figure01)Il


![An example for the confidence voting algorithm. Each row shows a part
                            of the output of three different voters. When choosing the most frequent
                            top result of each voter (bold) an I would be predicted. However,
                            when adding the confidences of each voter, the letter l is
                            predicted.](resources/images/figure01.jpg)
...
To obtain different voters based on a dataset several approaches are
                        ...
                        generated by a simple but robust approach called cross-fold-training #reul2018b
...
Calamari can easily be trained on new material if an appropriate model is not
                        ...
                        hyperparameters of Calamari are described in [Appendix B](#appendixb)
...
The smaller University of Washington Database III[^: http://www.tmbdev.net/ocrdata-split/]


The other large dataset German Text Archive of the 19th
                            Century[^: Published as part of the GT4HistOCR dataset , 
                                h]


The three historical printed books[^: Published as part of the GT4HistOCR
                            dataset , 
     ]


![Example lines of the used UW3, DTA19, 1476, 1478, and 1499 datasets.
                            Note that the last line is cropped.](resources/images/figure02.jpg)


An example line for each dataset is shown in [Figure
                            2](#figure02)[Table 1](#table01)[Sec. 4.1](#section4-1)


![Overview of the used datasets. The Codec size lists the number of
                            ...
                            evaluation set of UW3α is unknown.](resources/images/table01.JPG)
...
![CER using different software on the UW3 (20th cent. reports) and the
                            ...
                            The CERs on UW3α were published by Breuel (2017).](resources/images/table02.JPG)
...
The evaluation on the UW3 dataset shows that 97% of all lines are recognized
                        ...
                        degraded. [Figure 3](#figure03)


![Three of the worst recognized lines during the evaluation of Calamari
                            ...
                            cropped.](resources/images/figure03.jpg)


The second example has a degraded font, that is also larger than the other
                        lines. For example, waterswatorse


The last line is falsely cropped at the beginning, thus it is impossible to
                        recognize Energy Vol. 9


![Most common errors of Calamari on the UW3 evaluation set. The left
                            ...
                            characters are missing.](resources/images/table03.JPG)
...
Another crucial measure besides its accuracy for a good OCR system is its
                        ...
                        preprocessing of a single line is listed in [Table
                            4](#table04)


![Average time for training or prediction of a single line of the UW3
                            ...
                            row was published by Breuel (2017).](resources/images/table04.JPG)
...
The time for voting scales linearly with the number of models used as voters.
                        ...
                        approximately five times higher than the results shown in [Table 4](#table04)
...
Using the models trained on the UW3 or the DTA19 dataset to directly
                        ...
                        CERs are shown in [Table 5](#table05)
...
![Results on the historical printed books using Calamari trained with 50
                            ...
                            trained folds and the last column the voted result as CER.](resources/images/table05.JPG)
...
Calamari uses Adam #kingma2014


## Notes & References

Breuel, T. M. (2008) **The OCRopus open source OCR system****Document Recognition and Retrieval XV**
Breuel, T. M. (2017) **High performance text recognition using a hybrid
                        convolutional-lstm implementation****Document
                        Analysis and Recognition (ICDAR), 2017 14th IAPR International Conference
                        on**
Breuel, T. M., et al. (2013)
                        **High-performance OCR for printed English and Fraktur
                        using LSTM networks****Document Analysis and
                        Recognition (ICDAR), 2013 12th International Conference on**
Graves, A., et al. (2006)
                        **Connectionist temporal classification: labelling
                        unsegmented sequence data with recurrent neural networks****Proceedings of the 23rd international conference on Machine
                        learning**
Kingma, D. P. and BA, J. (2014)
                        **Adam: A method for stochastic optimization****arXiv preprint arXiv**
Pascanu, R., et al. (2013)
                        **On the difficulty of training recurrent neural
                        networks****International Conference on
                        Machine Learning**
Reul, C., et al. (2017a) **Case Study of a highly automated Layout Analysis and OCR of an
                            incunabulum:Der Heiligen
                        Leben(1488)****Proceedings of the 2nd
                        International Conference on Digital Access to Textual Cultural
                        Heritage**
Reul, C., et al. (2018a) **Improving OCR Accuracy on Early Printed Books by combining
                        Pretraining, Voting, and Active Learning****JLCL:
                        Special Issue on Automatic Text and Layout Recognition**
Reul, C., et al. (2018b) **Improving OCR Accuracy on Early Printed Books by utilizing
                        Cross Fold Training and Voting****2018 13th
                        IAPR International Workshop on Document Analysis Systems (DAS)**
Reul, C., et al. (2017b) **Transfer Learning for OCRopus Model Training on Early Printed
                        Books****Journal for Library Culture**
Springmann, U. and
                    Lüdeling, A. (2017) **OCR of historical printings with an
                        application to building diachronic corpora: A case study using the RIDGES
                        herbal corpus****Digital Humanities
                        Quarterly**
Springmann, U., et al.
                    (2018) **Ground Truth for training OCR engines on historical
                        documents in German Fraktur and Early Modern Latin****JLCL: Special Issue on Automatic Text and Layout
                        Recognition**
Srivastava, N., et al.
                    (2014) **Dropout: a simple way to prevent neural networks
                        from overfitting****The Journal of Machine
                        Learning Research**
Wick, C., et al. (2018) **Comparison of OCR Accuracy on Early Printed Books using the
                        Open Source Engines Calamari and OCRopus****JLCL:
                        Special Issue on Automatic Text and Layout Recognition**
Wiegand, F., et al. (2018)
                        **Recherchieren, Arbeiten und Publizieren im Deutschen
                        Textarchiv: ein Praxisbericht****Zeitschrift für
