...
history of computer programming for all*all*rankin2018*also*seriousprogramming for all

Using methods from critical code studies`LET``GOTO``GOSUB``GOTO``GOSUB`** Birth
of Basic ** A People’s
...
world than there are programmers
...
Time-Sharing System (DTSS) and the BASIC computer language
...
process and bringing computing to a wider audience programming
...
Hargraves developed the DART language on the newly-arrived LGP-30 computer
 were the leaders of the band; we weren’t the performers
...
of the largest, if not *the*kurtz2017bunsupportedfictitious charging scheme, undergraduates who
...
it was our machine — which we shared with John Kemeny
...
Queen ** Baa Baa Black Sheep ** In the Mood,**copeland_ ** Love Letters ** DARLING CHICKPEA, YOU ARE MY AVID
ENCHANTMENTM.U.C.** Spacewar!** Spacewar!** Rolling Stone ** Ready
or not, computers are coming to the people,
...
of entertainment student can challenge the computer to a game of
...
Lambert trophy. It's sort of a commemorative program.
...
in a highly realistic simulated game
...
students, which is an excellent way to learn programming ** Kiewit Comments ** and faculty have written several thousand programs,idle pasttimes
...
It’s perhaps no accident that ** Tennis for Two ** Pong.** ** Pong ** Pong.** ** Pong ** Tennis for
...
Yard Fight ** Tecmo Bowl.** ** John Madden
Football ** Madden ** Madden ** Madden **
...
being thrown twice in a quarter[^: Kemeny noted in the 1974 Pioneer Day
session that he wrote it on a Sunday ]*Dartmouth*The program is somewhat biased —
Dartmouth usually wins ** FOOTBALL ** Kiewit Comments.** ** FOOTBALL **
...
football **`GAME DELAYED. DOG ON THE
FIELD.`** It’s Different at
Dartmouth **[^: From this blog post, archived from Techmo Bowlers: https://bowlers102.rssing.com]JEAN’S
SPECIAL,**10 PRINT **montfort_ ** Acknowledgements ** BASIC Computer Games ** For recognizing games as a legitimate
...
make the obvious choice of IBM because they perce]-265[^: For a good
...
activity in a time-sharing system[^: They used the
...
people looked around for computers that we
...
time when computers will help call the next play.

BASIC was designed with a teletype interface rather than punch cards for
accessibility reasons along in the nick of
time-265Then we would set this up and the whole
...
the machine language writers and designers were men all cases where there is a choice
between simplicity and efficiency, simplicity is chosen[Kurtz 1981]()

Some examples of technical choices Kemeny and Kurtz made to simplify BASIC
include: hiding object code[^: From Kurtz memo to Annette Vee 9.29.17: Grammatical Simplifications in
BAS]

Commands in BASIC were designed to be *friendly*[Tribus memo]()`SAVE``HELLO``RUN``GOODBYE``LOGON``LOGOFF`[Marino]()[149-152]()natural languageHELLO
...
the compiler not to check line by line prior to compiling find the instantaneous
...
users had the helpful illusion that the computer was *theirs*kemeny_etal1968
...
pass and a half one exception — a
...
TO``GOTO``GOTO`** GOTO Considered Harmful **considered harmful`GOTO``GOTO`utterly unhelpful`GOTO`too much of an invitation to make a
mess of one’s program`GOTO``GOTO`** Communications of the ACM **`GOTO``GOTO`*new*
...
at the time. All functions were limited to three characters: `LET``INT``RND`unfortunate

As with every BASIC program of its time, each line of FTBALL contains an
instruction number (line #), operation (such as `LET``END`

The BASIC commands in use in FTBALL are (in the order they appear in the
program):
`REM`remarkscommentsthe reason is not recorded
for choosing the statement `REM``REMARK``PRINT``PRINT``PRINT``PRINT``INPUT`?`PRINT``INPUT``PRINT``GOTO``GOTO`unstructured programming`GOSUB``RETURN``GOSUB``KEEP
SCORE``LET``LET x=x+1``x=x+1``LET``FOR`the more natural testing for loop
completion *before*kurtz1981`NEXT``FOR``IF/THEN``ELSE``ELSE``IF`[FTBALL Lines 190 and
191]()`IF``THEN``ELSE``ELSE``RND`An important use of computers is the
...
of random events`RND``INT`floor`INT``STOP``END`
...
it was also coalescing around a masculine professional identity for all
...
Kemeny, J. and
Kurtz, T. (1968) ** Dartmouth Time-Sharing ** Science ** Kiewit
...
Montfort, N. (2005) ** Twisty Little Passages ** Birth of BASIC **[https://www.youtube.com/watch?v=WYPNjSoDrqw](https://www.youtube.com/watch?v=WYPNjSoDrqw)
