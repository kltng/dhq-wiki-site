...
Digital humanists who work with music will need tools for creating music notation that they can integrate into a
						...
						writing, USD $299 for Finale, $579 for Dorico, and $199 *per year*machine-readable#mei2022


When I found that these tools did not meet my needs, I turned to the notation software Lilypond, an open-source
						program for what its creators call digital music engraving#lilypond2022a#berry_etal2022
...
Part of the appeal of other document-preparation systems with plain-text input formats is that they make it
						...
						vision of minimal computing#risam_etal2022
...
Over the last ten years I used Lilypond (integrated with LaTeX) for a musicology dissertation, a monograph, and
						...
						more useful interface than is provided by default #cashner2017#cashner2020#cashner2021[ly2mei](https://github.com/andrewacashner/ly2mei)


#### 1.1 Lilypond, A Tool for Automated Music Typesetting


Lilypond is an open-source, freely available (libre#nienhuys_etal2003#lilypond2022b
...
Many others have written about the benefits of plain text, and this article is not meant to fan a flame war over
						tools #siddiqui2022#montalenti2016#sivers2022#tenen_etal2014#roberts2005The problem with most binary formats is that the context
							...
							application that created it#thomas_etal2020
...
In my case I could not afford a fast computer with a lot of storage, and so this workflow enabled me to use a
						...
						environment for writing, and I liked the promise of future-proofHuman-readable forms of data, and self-describing data, will outlive all other forms of data and the
							...
							it — potentially long after the original application that wrote it is defunct#thomas_etal2020
...
The following code sample ([Example 1](#example01)[Figure 1](#figure01)
							Lilypond input
\version "2.22"
\header {
...
The principle of semantic markup was central in the design of the LaTeX document-preparation system #berry_etal2022document classpackage
...
As an example, I wrote my book and editions in LaTeX using a custom class and several original packages that
						...
						default LaTeX, modeled after TEI #tei2022biblatex-chicago package automatically format a citation: `On the morning when 10,000 started out from Selma, the people sang about \quoted{that great gettin' up
								...
								morning.**Oh, Freedom****We Shall Overcome,**#southern1997`\usepackage{semantic-markup}``\sigla{D-Mbs}{Mus. ms. 1234}`
...
Compared with these systems for verbal texts, however, musical notation presents a challenge for semantic markup
						...
						pitches #steinberg2023house style
...
Lilypond’s creators understood the importance of creating a user interface that separated formatting from
						musical content. As they put it, Lilypond users have to key in the music by hand, so the input format is the
							user-interface to the program#nienhuys_etal2003as much non-musical
							information as possible, e.g., formatting instructionswe are left with a format that contains
							exactly the musical information of a piece#nienhuys_etal2003
...
To produce my editions I created a set of extensions to Lilypond that I call the lirio
						system (from the Spanish for lily[Appendix 6.1.1](#section06-1-1)lirio.) Lilypond does not provide any form
						...
						may become the basis for future efforts in this area #liska2020
...
Lilypond also provides a built-in extension language, Scheme, which enables users to create commands of much
						...
						concepts of lambda calculus #dybvig2009define command to create a function that takes a
...
Coloration brackets are not a default feature in any notation software I know of, and so any editor will need
							to create a solution. In a GUI system, I would want to be able to, say, highlight a group of notes and click a
								...
								divino**[Appendix 6.1.2](#section06-1-2)
MusicT = {
  \clef "treble"
  \MeterTriple
  | a'2 b'2 c''2
...
To draw the brackets, one option would be to use Unicode font characters in a font like Bravura that conforms
							to the Unicode music-notation specification known as SmuFL #steinberg2022`U+EA0C`mensuralColorationStartSquare) and `U+AE0D`mensuralColorationEndSquare). But introducing new fonts and dealing with character
...
`coloration-brackets.ly``\include "coloration-brackets.ly"``\color``\endcolor`[Figure 2](#figure02)[Subsection 2.2.1](#section02-2-1)
...
It would not be very efficient if I had to manually open dozens of files, export them to graphics, crop each
						...
						directory; then compiling the text document, which includes those images (see [Appendix 6.1.1](#section06-1-1)`make`pandoc to
...
Instead of generating the Lilypond output separately, there are a few ways to take the second approach listed
						...
						of LaTeX that incorporates the Lua scripting language, to integrate Lilypond in LaTeX in a streamlined way #peron_etal2019`\lilypond{}``\\begin{lilypond} ... \\end{lilypond}`OOoLilypond
...
There have been significant efforts in the last decades to develop ways of converting to and from the Lilypond
						...
						Scheme module in the openLilyLib, which manipulates Lilypond’s internal Scheme representation of the music file
							#berendson_etal2020#liska2020#napoleslopez_etal2019
...
Converting pitches, rhythms, and other basic notations is fairly straightforward. Since pitches in Lilypond
						...
						other markups; and a syllable of lyric text (see [Appendix 06.3.1](#section06-3-1)


Since this class also contains all the information needed for an MEI noteTMeiNoteRest type is used for an
						internal representation of an MEI node, and its constructor function knows how to create a node from a TPitch object (see [Appendix 06.3.2](#section06-3-2)XMLString of the resulting node will yield the proper MEI, such as the
...
The real challenge is in converting the structure of a Lilypond file to that of MEI. In Lilypond, the tree is
						...
						these branches to a new root for that measure, and then attaches all the measures to a common score root. [Appendix 6.3.3](#section06-3-3)
...
By the same token, though, scholars who opt not to keep up with the development of digital tools should not impose
					...
					in Microsoft Word and all music submissions in Finale or Sibelius. If journals would accept camera-ready#grunacher2022
