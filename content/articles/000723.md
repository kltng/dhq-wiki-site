---
title: "Problems of Authorship Classification: Recognising the Author Style or a Book?"
id: 000723
authors: ["František Válek", "Jan Hajič, Jr."]
volume: 017
issue: 4
date: 2024-01-12
type: article
keywords: ["machine learning", "literary studies"]
author_keywords: ["authorship attribution", "dataset design", "Czech", "experiment design", "delexicalisation"]
language: en
word_count: 9874
source: raw/articles/000723.xml
---
...
**Volume 017, Issue 4** (2024-01-12) | article | 9,874 words
...
Authorship attribution using machine learning is a fertile area of digital literary scholarship[^: See, for example, 
               , , , 
               , and  for general studi][^: Stylo: 
               Stylometric Multivariate Analyses, available at 
        ]Robert Gorman#gorman2022Efstathios StamatatosKim Luyckx#stamatatos2009#luyckx2011


In this article, we show that in the domain of long-form literary works, stylistic variation between individual works 
               ...
               to attribute texts with unclear authorship, such as in the seminal study of **The Federalist Papers**Frederick MostellerDavid L. Wallace#mosteller_wallace1964


Our findings are applicable when we are interested in attributing texts to authors *despite*


The contribution of our article can be summarised thus: the stylistic variation between individual books of an author 
               is significant enough to affect state-of-the-art authorship classification system performance. Thus, to credibly
               claim a certain level of ability to classify the style of an *author**books*


It should be noted that we did *not*#tyo_dhingra_lipton2022


Crucially, as in #gorman2022delexicalisationauthorial style


It should be noted that the use of non-lexical features is by no means rare in computational stylometry 
               #swain_mishra_sindhu2017#nivre2015#gorman2022


In the rest of the article, we first introduce our dataset and specify pre-processing and hyperparameter search procedures 
...
The following visualisation summarises our findings. The columns show results for different segmentation lengths 
...
Our dataset consists of 210 books (written in Czech) by 23 authors (for a full overview, see the table in the appendix). 
               ...
               number of books (ranging from 4 books by Č. SlepánekK. Čapek


Such a diverse and unbalanced nature dataset may not be ideal for machine learning (ML) experiments, but it reflects the 
               ...
               contrasted with the dataset of #gorman2022[^: Compare also with , who focus on the effect of author set 
               size a]
...
Second, the books we have chosen vary greatly in length. #gorman2022Č. SlepánekA. Jirásek


Finally, our dataset includes mainly prose (mostly literary, but also journalistic and scholarly), a few works of drama, 
...
The raw data we have at our disposal are scanned books that have been processed with optical character recognition 
                   (OCR).[^: We have used the digital collections of the National Library of the Czech Republ][^: The raw 
                   data consisted of individual pages as .txt files wit][^: Available at:  
...
For training and testing authorship detection, we must split the books into shorter segments. For testing, we 
...
We split the dataset into segments of 1000, 500, 200, 100, and 50 tokens as data points for classification 
                   ...
                   lengths represent the *average*


Specifically, we have pre-split the data into traindevelopmenttest[^: In machine learning 
                   experiments, the development set is used]trainingtraindevelopment
...
As stated above, we have delexicalised the dataset using the publicly available Application Programming Interface (API) 
                   of UDPipe at LINDAT/CLARIAH.[^: Available at: 
                   
                   https://lindat.mff.cuni.cz][^: Autosemantic words, 
                   as recognized by UDPipe, are: nouns (NOU][^: See 
                   
                   http://universaldependencies.org/doc]


In contrast, #gorman2022#swain_mishra_sindhu2017


In addition to using the above mentioned form of delexicalisation, we have performed a variety of delexicalisations 
                  ...
                  the approach utilized by #gorman2022
...
Because these experiments are essentially a grid search, we have selected only a subset of our data, consisting of 
                 six authors (5 books per each, 30 in total): A. StašekJ. NerudaJ. ArbesK. KlostermannF. X. ŠaldaT. G. Masaryk[^: These are the books 
                 designated as b-01 to b-05 for each of the]


For these experiments, we chose one of 10 different levels of delexicalisation. All of these pre-processings have 
                 been segmented in the same way as the larger dataset used for the rest of the experiments (1000, 500, 200, 100, and 50 tokens).


The different modes of delexicalisation were abbreviated as r-codes[^: Codes r-01, r-02, and 
                 r-03 were used in preparation for furthe][^: Aside from 
                 the word form for non-delexicalised baselines, we h]#strakova_straka_hajic2019[^: The types of named entities are persons 
                 (first names and surna]
                     r-04: No delexicalisation (baseline) — original word forms are usedr-05: Lemmatisation — lemmas used instead of word formsr-06: Part-of-speech tags for all wordsr-07: Morphological tags for all words r-08: Part-of-speech tags for autosemantic words, others lemmatisedr-09: Morphological tags for autosemantic words, others lemmatisedr-10: NameTag tags for recognised named entities, others with original word formsr-11: NameTag tags for recognised named entities, others lemmatisedr-12: NameTag tags for recognised named entities, part-of-speech tags for autosemantic words, others lemmatisedr-13 NameTag tags for recognised named entities, morphological tags for autosemantic words, others lemmatised
...
We then conducted a series of experiments across all of these levels of delexicalisation as well as across different 
                 ...
                 #savoy2020#pedregosa_et_al2011
                     Naive Bayes (sklearn.naive_bayes.MultinomialNB)C-Support Vector Classification (sklearn.svm.SVC)Linear Support Vector Classification (sklearn.svm.LinearSVC)K-Nearest Neighbours (sklearn.neighbors.KNeighborsClassifier)Stochastic Gradient Descent (sklearn.linear_model.SGDClassifier)Decision Tree (sklearn.tree.DecisionTreeClassifier)


We used the same feature extraction settings for each (sklearn.feature_extraction.text.CountVectorizer). The only adjusted 
                 setting was word n-gram size, set to unigrams, bigrams, and trigrams 
                 (vectorizer = CountVectorizer(ngram_range=(n_min, n_max))). The training was performed using only the traintestdevel
...
These initial experiments have shown a performance dependency on different levels of delexicalisation. In addition, it 
                ...
                that we are not recognising the author style *per se*
...
Using the pre-processing/classification pipeline described above, we performed three experimental steps to illustrate 
                the relationship between author and book style in authorship classification:
                      
                        The full dataset (see above; 23 authors, 210 books) was used for a task of authorship attribution with 
                           *each book*traintesteasyNext, we performed the experiment with the same settings, but this time we built the test*one book from each author**all its segments*harderFinally, we performed the same experiment as outlined in #1, but instead of classifying by author, we classified 
...
We again emphasise that our purpose here is not to reach the best possible classification accuracy but rather to explore the 
                influence of authorial style variation between individual works on the classification accuracy of a decent
...
In addition to measuring performance on the train+devel versus testtrain+test vs. devel
...
The selection of individual segments as testing seems to have only limited influence on the results. The best results 
                  ...
                  the performance.[^: There is clearly space for improving classification accuracy here; the features ]
...
Interestingly, when using the segments of 1000 tokens, the accuracy of the classifier was consistently around the same 
                  ...
                  However, when shortening the segments, the performance on the larger dataset radically dropped.[^: For the effect of 
                  author set size and data size in authorship]


The results of this first experiment serve as a baseline for the second, where we show how setting aside specific 
...
We believe that the above-mentioned experiments are relatively simple ML-based author attribution tasks. In our opinion, 
                  the experimental settings that use the same books for training and testing, such as #gorman2022#benotto2021#swain_mishra_sindhu2017


Therefore, we have further expanded the experimental scenario to address real-life problem: recognising a book (or rather 
...
We randomly selected five sets of testing books such that each set contained one book from each author and no book was in 
                  two testing sets, except for **Svědomí Lidových novin, čili, Jak bylo po léta v českém tisku štváno 
                  lživě proti mně**Č. Slepánek
...
The results show a drop of 0.04 to 0.23, with 0.10 being the average deterioration of classification accuracy. In the 
...
We note that performing five-fold, cross-validation with 23-book test sets rather than 210-fold, leave-one-out, 
...
Compared to the leave-one-out setting, the effect of removing books potentially helpful for identifying an author from the
                  ...
                  [^: Specifically, the 11 test books where we compared the 5-fold and leave-one-out a]


These experiments show that there is little to be gained by performing the remaining 199 leave-one-out experiments over 
...
A possible systematic confounding factor for the drop in average performance could be the irregularity of training set 
                  ...
                  Across Bookstraintesttraintesttesttraintest ratiotest

|  | All Tokens | Set 1Rest RatioAccuracy | Set 2 | Set 3 | Set 4 | Set 5 |
| --- | --- | --- | --- | --- | --- | --- |
...
The following table shows the correlation coefficients of the authors' accuracies to the traintesttesttrain
...
On the other end of the data size spectrum, K. SabinaJ. Arbes**Persekuce lidu českého v letech 1869-1873**ArbesV. Hálek**Fejetony**
...
This further opens the question of the influence of genres on the performance of the models. In general, there are 
                  ...
                  of drama were used for testing in the case of four authors on eight instances[^: E. Krásnohorská (a-09) 
                  in Set 2; J. Vrchlický (a-12) in sets ][^: V. Hálek 
                  (a-21) in Set 4; see appendix.]K. ČapekJ. Vrchlický


On the other hand, for V. HálekK. Sabina


There are several other cases where the influence of the selected test book can be well explained. For example, 
                  V. Hálek**Na statku a v chaloupce****Kresby křídou i tuší**
...
Our third experiment aimed to discover the structure of stylistic similarity within individual authors. As hinted by the 
...
To expose these characteristics, we ran the classification pipeline with the 210 individual books (instead of the 23 authors) 
                  ...
                  books, especially in the easy
                    The greater the ratio of b / (a + b), the more consistent an author's style is.The greater the ratio of a / (b + c), the more inconsistent an author's style.The greater the ratio of (a + b) / (a + b + c), the more distinctive an author is.


We have split each book into 80% of training and 20% of test segments (see above). In this step, two sets of experiments 
                  were run. In the first, only books with over 20,000 tokens were used to ensure decent model training (see, for example,  
                  #gorman2022


![Results of classification of books by individual authors, in numbers of segments attributed to the correct book 
                     (blue), other books of the same author (yellow), and books of a different author (red); created using Flourish 
                     (https://flourish.studio/, accessed 5 April 2023)](resources/figure01.png)


These results may be compared with the experiments performed in Step 2. The author a-20 (S. K. NeumannT. G. Masaryk


![Heatmaps of intra-author classification for a-20 and a-06; created using Flourish 
                     (https://flourish.studio/, accessed 5 April 2023)](resources/figure02.png)
...
Some interesting phenomena revealed by this experiment may be noted. The results show that accuracy scores are not strongly 
...
From this, one may assume that collections of short stories are good candidates for high levels of intra-author confusion. 
                  However, some examples contradict this assumption. For example, a-15.b-02, 03, 04, and 06 are collections of short stories 
                  by K. Čapek


Another interesting example is a high level of confusion among books a-20.b-06, 07, and 08 (see the image above). These 
                  three books form a trilogy together (**Francouzská revoluce**
...
|  | Proportion of the Test Book Segments Attributedto the Correct Author but an Incorrect Book (Step 3) | Author's Performance (Step 2) |
| --- | --- | --- |
...
We do not claim to have studied the issue of book versus authorial style exhaustively. What we have done is build a pipeline 
...
Realised with the support of **Institutional Research**National Library of the Czech RepublicMinistry of Culture**Long-Term Conception 
               Developement of Scientific Organisation**


### Appendix

| Author | Title | Genre | Book ID in Dataset | Token Count | Set in Which Usedas Test Book(Experiment Step 2) |
| --- | --- | --- | --- | --- | --- |
...
| Č. Slepánek | Svědomí Lidových novin, čili, Jak bylo po léta v českémtisku štváno lživě proti mně | Prose | a-08.b-03 | 6,004 | Set, Set 5 |
...
Benotto, G. (2021) **Can an author style be unveiled through 
               word distribution?****Digital Humanities Quarterly**[https://dhq.digitalhumanities.org/vol/15/1/000539/000539.html](https://dhq.digitalhumanities.org/vol/15/1/000539/000539.html)
Gorman, R. (2022) **Universal dependencies and author 
               attribution of short texts with syntax alone****Digital Humanities Quarterly**[https://dhq.digitalhumanities.org/vol/16/2/000606/000606.html](https://dhq.digitalhumanities.org/vol/16/2/000606/000606.html)
Grieve, J. (2005) **Quantitative authorship attribution: 
               A history and an evaluation of techniques**[https://summit.sfu.ca/item/8840](https://summit.sfu.ca/item/8840)
Grzybek, P. (2014) **The emergence of stylometry: 
               Prolegomena to the history of term and concept****Text 
               within text: Culture within Culture**
Holmes, D.I. (1998) **The evolution of stylometry in 
               humanities scholarship****Literary and Linguistic Computing**
Kusakci, A.O. (2012) **Authorship attribution using 
               committee machines with k-nearest neighbors rated voting****Proceedings of the 11th symposium 
               on neural network applications in electrical engineering, IEEE, 2012**[https://ieeexplore.ieee.org/document/6419997](https://ieeexplore.ieee.org/document/6419997)
Luyckx, K., and Daelemans, W. (2011) 
               **The effect of author set size and data size in authorship attribution****Literary and Linguistic Computing**
Luyckx, K. (2011) **Scalability issues in authorship 
               attribution**
Marino, V.Q., Hirst, G., and Amancio, D.R. 
               **Authorship attribution via network motifs identification****Proceedings
               of the 5th Brazilian conference of intelligent systems, IEEE, 2016**[https://doi.org/10.48550/arXiv.1607.06961](https://doi.org/10.48550/arXiv.1607.06961)
Mosteller, F., and Wallace, D. (1964) 
               **Inference and disputed authorship: The Federalist**
Nivre, J. (2015) **Towards a universal grammar for
               natural language processing****Proceedings of the international conference on intelligent
               text processing and computational linguistics, CICLing, 2016**[https://doi.org/10.1007/978-3-319-18111-0_1](https://doi.org/10.1007/978-3-319-18111-0_1)
Nutanong, S. et al. **A scalable framework
               for stylometric analysis query processing****Proceedings of the 16th international
               conference on data mining, IEEE, 2016**[https://doi.org/10.1109/ICDM.2016.0147](https://doi.org/10.1109/ICDM.2016.0147)
Pedregosa, F. et al. (2011) **Scikit-learn:
               Machine learning in Python****Journal of Machine Learning Research**[https://jmlr.csail.mit.edu/papers/volume12/pedregosa11a/pedregosa11a.pdf](https://jmlr.csail.mit.edu/papers/volume12/pedregosa11a/pedregosa11a.pdf)
Pinho, A.J., Pratas, D., and Ferreira, P.J.S.G.
               **Authorship attribution using relative compression****Proceedings of the
               data compression conference, IEEE, 2016**[https://doi.org/10.1109/DCC.2016.53](https://doi.org/10.1109/DCC.2016.53)
Ramezani, R., Sheydaei, N., and Kahani, M. (2013)
               **Evaluating the effects of textual features on authorship attribution accuracy****Proceedings of the international econference on computer and knowledge engineering, IEEE, 
               2016**[https://doi.org/10.1109/ICCKE.2013.6682828](https://doi.org/10.1109/ICCKE.2013.6682828)
Savoy, J. (2020) **Machine learning methods for
               stylometry: Authorship attribution and author profiling**
Segarra, S., Eisein, M., and Ribeiro, A. (2013)
               **Authorship attribution using function words adjaceny networks****Proceedings of the international conference on acoustics, speech and signal processing, IEEE, 2013**[https://doi.org/10.1109/ICASSP.2013.6638728](https://doi.org/10.1109/ICASSP.2013.6638728)
Stamatatos, E. (2009) **A survey of modern
               authorship attribution methods****Journal of the American Society for Information 
               Science and Technology**[https://doi.org/10.1002/asi.21001](https://doi.org/10.1002/asi.21001)
Straková, J., Straka, M., and Hajič, J. (2019)
               **Neural architectures for nested NER through linearization****Proceedings
               of the 57th annual meeting of the association for computational linguistics, ACL, 2019**[https://aclanthology.org/P19-1527.pdf](https://aclanthology.org/P19-1527.pdf)
Swain, S., Mishra, G., and Sinhu, C. (2017)
               **Recent approaches on authorship attribution techniques: An overview****Proceedings of the international conference of electronics, commmunication, and aerospace
               technology, ICECA, 2017**[https://doi.org/10.1109/ICECA.2017.8203599](https://doi.org/10.1109/ICECA.2017.8203599)
Straka, M. (2018) **UDPipe 2.0 prototype at CoNLL 2018 
               UD shared task****Proceedings of the CoNLL 2018 shared task: Multilingual parsing from raw
               text to universal dependencies, ACL, 2018**[https://doi.org/10.18653/v1/K18-2020](https://doi.org/10.18653/v1/K18-2020)
Tyo, J., Dhingra, B., and Lipton, Z.C. (2022)
               **On the state of the art in authorship attribution and authorship verification****arXiv**[https://doi.org/10.48550/arXiv.2209.06869](https://doi.org/10.48550/arXiv.2209.06869)
Zhao, Y, and Zobel, J. (2007)
               **Searching with style: Authorship attribution in classic literature****Proceedings of the 30th Australasian computer science conference, ACSC, 2007**[https://www.researchgate.net/publication/221574042_Searching_With_Style_Authorship_Attribution_in_Classic_Literature](https://www.researchgate.net/publication/221574042_Searching_With_Style_Authorship_Attribution_in_Classic_Literature)
